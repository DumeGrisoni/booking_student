    ...
    <>
      ...
      >
        ...
        <div className="flex items-center justify-center lg:justify-between h-full gap-[30px]">
          ...
          {/* LINKS */}
          <ul
            className={`lg:flex-1 lg:relative absolute justify-around items-center flex-col lg:flex-row gap-[30px] lg:mt-0 mt-16 transition-all lg:duration-100 duration-300 ease-in ${
              !isOpen ? 'flex' : 'top-[-500px]'
            } `}
          >
            {Links.map((link) => (
              <li key={link.name} className="lg:py-0 py-1">
                <NavLink
                  to={link.link}
                  exact
                  className="hover:text-primary-var-1 text-title lg:font-normal font-bold text-grey-font transition-all duration-100"
